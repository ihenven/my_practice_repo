...
var printBoard = function () {
    for (var row = 0; row < 9; row++) {
        for (var col = 0; col < 9; col++) {
            var cellId = `r${row + 1}${col + 1}`; 
            var cell = document.getElementById(cellId);
            if (cell) {
                cell.textContent = sol[row][col] !== 0 ? sol[row][col] : ""; 
            }
        }
    }
};
...
var loadNewPuzzle =  function() {
    fetch("https://sudoku-api.vercel.app/api/dosuku") .then(response => response.json()).then(data => {
            if (data && data.newboard && data.newboard.grids && data.newboard.grids[0]) {
                let newPuzzle = data.newboard.grids[0].value; 
                sol = newPuzzle; 
                printBoard(); }
        })
        .catch(error => console.error("Error loading new puzzle:", error));
};
