...
'''hw1.py has five functions that must be implemented by you. A description of what the
function should do is written in the comments above each function. Make sure to read
the comments carefully.
3. Implement each function and run the program. Calls to each function with test data and
assert statements are included at the bottom of the file to test your work. You should
see “correct” printed for each assert statement. If an error is thrown, go back and check
your work.'''
...
def has123(nums):
    """
    Given a list of ints, return True if the sequence of numbers
...
def centered_avg(nums):
    ...
    if  y%2 == 0:
        nums.sort()
    centered_avg = nums[1:-1]
    return sum(centered_avg) // len(centered_avg)
